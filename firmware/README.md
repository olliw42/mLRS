...
- the pre-release needs to be commited with, e.g., commit message "v1.3.06 (pre-release)" and tagged with, e.g., "v1.3.06-pre-release"
- the tag needs to be added to the file tools/web/mlrs_firmware_urls.json, so that flash tools can find it
...
- the filenames include the SHA with a "-@", such as "-@ae667b78". This is used by flash tools to identify dev versions. ALL files in the firmware folder need to have identical SHA.
- dev release need NOT be tagged, they are identified by being in main branch and having the SHA in the file name
