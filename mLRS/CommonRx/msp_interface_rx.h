...
class tRxMsp
{
  ...
  private:
    ...
    bool msp_ident_waiting_for_response;
    bool gcs_connected;
...
void tRxMsp::Init(void)
{
    ...
    msp_ident_waiting_for_response = false;
    gcs_connected = false;

    memset(inav_flight_modes_box_mode_flags, 255, INAV_FLIGHT_MODES_COUNT); // 255 = is empty>>>>>>> dev-mspv1
}
    ...
    if (fifo_link_out.HasSpace(MSP_FRAME_LEN_MAX + 16)) { // we have space for a full MSP message, so can safely parse
        while (serial.available()) {
            ...
            if (msp_parse_to_msg(&msp_msg_ser_in, &status_ser_in, c)) {
                ...
                if (gcs_connected && msp_ident_waiting_for_response) { // filter out all messages which are not
                    send = false;
                    if (msp_msg_ser_in.function == MSP_IDENT) { // can be response or error
                        msp_ident_waiting_for_response = false;
                        send = true;
                    }
                } else
...
*/
    ...
    if (gcs_connected && (tnow_ms - msp_ident_tlast_ms) >= 3500) {
    ...
    if (msp_parseX_to_msg(&msp_msg_link_in, &status_link_in, c)) { // converting from mspX
        ...
        if (msp_msg_link_in.type == MSP_TYPE_REQUEST) { // this is a request from a gcs
            ...
            if (msp_msg_link_in.function == MSP_IDENT) { // we got a MSP_IDENT, so there is a gcs
                ...
                gcs_connected = true;
                msp_ident_waiting_for_response = true;
            }
        }
/*
dbg.puts("\n");
