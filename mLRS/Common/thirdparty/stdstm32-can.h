...
void can_init(void)
{
    ...
    dbg.puts("\n  SJW: ");dbg.puts(u8toBCD_s(timings.sync_jump_width));)
    DBG_DC(
    tDcHalCanTimings APtimings;
    uint16_t sample_point_permill;
    ap_compute_timings(peripheral_clock_rate, 1000000, &APtimings, &sample_point_permill);
    dbg.puts("\n  AP Prescaler: ");dbg.puts(u16toBCD_s(APtimings.bit_rate_prescaler));
    dbg.puts("\n  AP BS1: ");dbg.puts(u8toBCD_s(APtimings.bit_segment_1));
    dbg.puts("\n  AP BS2: ");dbg.puts(u8toBCD_s(APtimings.bit_segment_2));
    dbg.puts("\n  AP SJW: ");dbg.puts(u8toBCD_s(APtimings.sync_jump_width));
    dbg.puts("\n  AP %%%: ");dbg.puts(u16toBCD_s(sample_point_permill)););
