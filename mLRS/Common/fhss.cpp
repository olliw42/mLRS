...
void tFhssBase::generate(uint32_t seed)
{
    ...
    while (k < cnt) {
        ...
        if ((config_i != SX_FHSS_CONFIG_FREQUENCY_BAND_433_MHZ &&
             config_i != SX_FHSS_CONFIG_FREQUENCY_BAND_866_MHZ_IN &&
             config_i != SX_FHSS_CONFIG_FREQUENCY_BAND_900_MHZ_TAIWAN) && (k > 0)) { // TODO: use smarter method, e.g., cnt < 2/3
