...
void sx126x_rfpower_calc(const int8_t power_dbm, uint8_t* sx_power, int8_t* actual_power_dbm, const uint8_t frequency_band)
{
    ...
    if (power_dbm >= POWER_27_DBM) {
        *sx_power = (frequency_band == SETUP_FREQUENCY_BAND_868_MHZ) ? -3 : -1;
        *actual_power_dbm = 27;
    } else
    if (power_dbm >= POWER_24_DBM) {
        *sx_power = (frequency_band == SETUP_FREQUENCY_BAND_868_MHZ) ? -6 : -4;
        *actual_power_dbm = 24;
    } else
    if (power_dbm >= POWER_22_DBM) {
        *sx_power = (frequency_band == SETUP_FREQUENCY_BAND_868_MHZ) ? -8 : -6;
        *actual_power_dbm = 22;
    } else {
        *sx_power = -9;
        *actual_power_dbm = (frequency_band == SETUP_FREQUENCY_BAND_868_MHZ) ? 20 : 17;
    }
}
